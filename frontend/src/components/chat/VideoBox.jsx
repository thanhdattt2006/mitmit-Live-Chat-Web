...
import { Mic, MicOff, Video, VideoOff, Heart, ArrowRight, Loader2 } from 'lucide-react';
...
export default function VideoBox() {
  const { lang, isMatching, startMatching, setConnected, decreaseQuota, addMessage, clearMessages } = useStore();
  ...
  return (
    <section className="flex-1 relative rounded-3xl overflow-hidden bg-neutral-200 dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800 shadow-sm flex items-center justify-center h-full">
      ...
      <div className={`absolute inset-0 bg-neutral-900/80 backdrop-blur-md flex flex-col items-center justify-center transition-opacity duration-300 text-white z-10 ${isMatching ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 glass-panel p-2 rounded-full border border-white/20 dark:border-white/10 shadow-2xl flex items-center gap-2 z-10">
        ...
        <button 
          ...
          className={`w-12 h-12 rounded-full flex items-center justify-center text-white backdrop-blur-md transition-all active:scale-90 ${isCamOn ? 'bg-white/20 dark:bg-black/40 hover:bg-white/40 dark:hover:bg-black/60' : 'bg-red-500/80'}`}
        >
          {isCamOn ? <Video className="w-5 h-5 fill-current" /> : <VideoOff className="w-5 h-5 fill-current" />}
        </button>
