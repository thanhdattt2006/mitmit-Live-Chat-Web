    ...
    try {
      ...
      if (type === 'ban') {
        await axiosClient.post(
          `/api/v1/admin/ban/${reportedId}?reportId=${reportId}`,
        );
  ...
  const StatCard = ({ icon: Icon, label, value, color }) => (
    <div className='relative overflow-hidden bg-neutral-900/40 backdrop-blur-xl border border-white/10 rounded-3xl p-6 transition-all hover:scale-[1.02] hover:bg-neutral-900/60 group'>
      ...
      <div className='flex items-center gap-4 relative z-10'>
        <div
          className={`w-14 h-14 bg-${color}-500/10 rounded-2xl flex items-center justify-center shrink-0 border border-${color}-500/20`}
        >
      ...
      <div className='flex-1 overflow-y-auto p-6 lg:p-10 relative'>
        ...
        <div className='max-w-6xl mx-auto relative z-10'>
          <header className='mb-10 flex items-center justify-between'>
            ...
            <button
              onClick={() => navigate('/')}
              className='lg:hidden p-3 bg-white/10 rounded-xl'
            >
              <ArrowLeft />
            </button>
          </header>
                        ...
                        >
                          ...
                          <td className='px-8 py-5 text-right space-x-3'>
                            <button
                              ...
                              className='px-4 py-2 rounded-xl text-sm font-semibold text-gray-400 hover:text-white hover:bg-white/10 transition-all'
                            >
                              {t.IGNORE}
                            </button>
                            <button
                              onClick={() =>
                                handleAction('ban', r.id, r.reportedId)
                              }
                              className='inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold text-white bg-rose-600 hover:bg-rose-500 shadow-lg shadow-rose-500/20 transition-all hover:-translate-y-0.5'
                            >
                          ...
                          <td className='px-8 py-5 text-right space-x-3'>
                            {u.status === 'BANNED' && (
                              <button
                                ...
                                className='inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-500 shadow-lg shadow-emerald-500/20 transition-all hover:-translate-y-0.5'
                              >
